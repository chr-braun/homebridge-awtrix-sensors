...
function onSensorSelected() {
    ...
    showTab(tabName) {
        ...
        // Load sensors if switching to sensors tab
        if (tabName === 'sensors') {
            this.loadDiscoveredSensors();
        }
    }
    ...
    async searchMqttSensors() {
        ...
        if (!searchBtn || !searchStatus) return;
        ...
        try {
            ...
            const mqttConfig = {
                host: document.getElementById('mqtt-host')?.value || '192.168.178.29',
                port: parseInt(document.getElementById('mqtt-port')?.value) || 1883,
                username: document.getElementById('mqtt-username')?.value || 'biber',
            ...
            if (sensorsTab) {
                sensorsTab.style.display = 'block';
            }
            ...
            console.error('Sensor search error:', error);
            searchStatus.className = 'search-status error';
            searchStatus.innerHTML = 'Fehler bei der Sensor-Suche: ' + error.message;
        } finally {
            // Reset button
    ...
    loadDiscoveredSensors() {
        ...
        if (this.discoveredSensors.length === 0) {
                ...
                <div class="sensor-meta">
                    <div class="quality-indicator">
                        ...
                        <div class="quality-bar">
                            <div class="quality-fill" style="width: ${qualityWidth}%; background: ${qualityColor};"></div>
                        </div>
                        <span>${sensor.quality}%</span>
                    </div>
                    <div class="last-seen">${sensor.lastSeen}</div>
                </div>
            </div>
        `;
    ...
    getQualityColor(quality) {
        if (quality >= 90) return '#28a745';
        if (quality >= 70) return '#ffc107';
        return '#dc3545';
    }

    // Select sensor card
    selectSensorCard(card, index) {
        // Remove previous selection
        document.querySelectorAll('.sensor-card').forEach(c => c.classList.remove('selected'));
        
        // Select current card
        card.classList.add('selected');
        this.currentSensor = this.discoveredSensors[index];
    }
    ...
    // Save sensors to configuration
    saveSensors() {
        if (this.discoveredSensors.length === 0) {
            this.showToast('Keine Sensoren zum Speichern gefunden', 'error');
            return;
        }
        
        // Update configuration with discovered sensors
        this.config.discoveredSensors = this.discoveredSensors;
...
function showTab(tabName) {
    if (window.awtrixConfigUI) {
        window.awtrixConfigUI.showTab(tabName);
    }
}

function searchMqttSensors() {
    if (window.awtrixConfigUI) {
        window.awtrixConfigUI.searchMqttSensors();
    }
}

function refreshSensors() {
    if (window.awtrixConfigUI) {
        window.awtrixConfigUI.refreshSensors();
    }
}
